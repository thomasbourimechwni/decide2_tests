...
def graph_read(file_input, file_output, aws_class):
    lines = [line.rstrip('\n') for line in open('read/' + file_input)]
    del lines[0]
    times = []
    nb_elt = []
    ...
    plt.tick_params(labelsize='6')
    plt.grid()
    plt.title('PRECO2 data extract duration with AuroraPsql on %s instance ' %aws_class)
...
if __name__ == '__main__':
    ...
    #graph("awsAuroraPsqlDbCreation.log","aurora_psql_result.png")
    graph_read("read_chunk_1095_r4large.log", "aurora_psql_read_r4large.png", 'r4large')
